...
select ENumber tEmployee,DNumber,EName,StartDate from tEmployee;
#3. 직원 테이블에서 직급코드를 출력하되, 각 항목이 중복되지 않게 출력하라.
select distinct (RNumber) from tEmployee;
#4. 생산량이 500 이상인 생산의 직원코드와 제품코드를 출력하라.
select ENumber,PNumber from tProduction where PCount >= 500;
...
select IName, Price from tItem where not (500<=Price and Price<=1000);
#7. 2010-05-01 이전에 입사한 직원의 직급코드, 이름, 주소, 입사일 출력하되, 입사일을 기준으로 오름차순으로 정렬하라. - to_date(’2010-05-01’)
select RNumber, EName, EAddr, StartDate from tEmployee order by StartDate;
#8. 부서 코드가 ‘D2001’, ‘D4001’ 에 속하는 모든 직원의 이름과 부서 코드를 출력하되, 이름을 기준으로 내림차순으로 정렬하라.(desc)
select EName,DNumber from tEmployee where DNumber = 'D4001' or DNumber = 'D2001' order by EName desc;
#9. 부서 코드가 ‘D2001’, ‘D4001’ 에 속하는 모든 직원 중 입사일이 ‘2010-05-01’ 이전에 입사한 직원의 이름과 부서 코드, 입사일을 출력하라.
select EName,DNumber,StartDate from tEmployee where (DNumber = 'D4001' or DNumber = 'D2001') and (StartDate <= '2010-05-01');;
...
select IName,Price from tItem where (500<=Price and Price<=1500) and (INumber like 'I1001' or INumber like 'I1002' or INumber like 'I1003');
#15. 테이블에서 반품 수량이 100 이상이고, 반품 사유 코드(RRNumber)가 'RR0001' 또는 'RR0002'인 반품의 반품번호와 반품일자를 출력하라.
select ONumber,RDate from tReturn where RCount >= 100 and (RRNumber like 'RR0001' or RRNumber like 'RR0002');
#16. 직급코드가 R0001, R0002, R0003 이고, 부서코드가 D1001이 아닌 직원의 이름, 직급코드, 주소를 출력하라.
select EName,DNumber,EAddr from tEmployee where (RNumber like 'R0001' or RNumber like 'R0002' or RNumber like 'R0003') and DNumber not like 'D1001';
