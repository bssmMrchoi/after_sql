...
select EName from tEmployee e join tDepartment d on e.DNumber = d.DNumber
where d.DName in (select DName from tEmployee where DName = '전자기기생산부') and e.StartDate >= '2020-01-01';
...
select EName, DName from tEmployee e left outer join tDepartment d on e.DNumber = d.DNumber;
#2. 생산 테이블과 제품 테이블을 조인하여 생산일자와 함께 생산된 제품의 이름과 수량을 출력하라.
select PDate,IName,PCount from tProduction p join tItem i on p.INumber = i.INumber;
...
select EName,PCount from tEmployee e join tProduction p on e.ENumber=p.ENumber group by PNUmber;
...
select CName,IName,RCount from tCustomer c join tOrder o on c.CNumber=o.CNumber join tProduction p on o.PNumber=p.PNumber join tReturn r on o.ONumber=r.ONumber join tItem i on i.INumber=p.INumber;
