...
select h.name, max(p.age) from hospital h join appointment a on h.hospital_id = a.hospital_id join patient p on p.patient_id = a.patient_id group by h.name;
#13. ‘서울 중앙 병원’에 예약한 환자 목록을 예약일시 내림차순으로 출력하시오.
#14. 예약이 한 건도 없는 병원의 ID와 이름을 출력하시오.
select h.hospital_id, h.name from hospital h where h.hospital_id not in (select a.hospital_id from appointment a);