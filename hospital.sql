...
select p.name, a.reservation_datetime from patient p join appointment a on p.patient_id = a.patient_id group by p.name, a.reservation_datetime having max(a.reservation_datetime);