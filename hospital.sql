...
select h.hospital_id, h.name, count(*) from hospital h join appointment a on h.hospital_id = a.hospital_id group by h.hospital_id, h.name order by count(*) asc limit 1;
#15. 병원별 예약된 고유 환자 수를 출력하시오. (같은 환자가 여러 번 예약했어도 1명으로 집계)
select h.name, count(distinct patient_id) from hospital h join appointment a on h.hospital_id = a.hospital_id group by h.name;