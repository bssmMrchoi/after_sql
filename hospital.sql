...
select h.name, h.hospital_id, count(a.patient_id) as cnt
from hospital h join appointment a
...
from appointment a join hospital h
...
from hospital h left join appointment a
on h.hospital_id=a.hospital_id
where a.patient_id is Null;

15. 병원별 예약된 고유 환자 수를 출력하시오. (같은 환자가 여러 번 예약했어도 1명으로 집계)
select h.name, count(distinct p.name)
from patient p join appointment a
on p.patient_id=a.patient_id
