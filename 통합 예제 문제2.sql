...
select EName, date_format(StartDate, '%Y-%M-%D') from tEmployee where (DNumber = 'D1001' or DNumber = 'D2001') order by StartDate;
#2. 제품 별로 생산량 최저, 최고, 총, 평균을 구하라. (min(), max(), sum(), avg())
select min(PCount), max(PCount), sum(PCount), avg(PCount) from tProduction;
#3. 생산량 조정을 위해 2020년 2월의 총 생산량을 알려고 한다. 해당 월에 생산된 제품들의 코드와 해당 제품들의 총 생산량을 출력하시오. (총 생산량의 오름차순 출력) - sum()
select INumber, sum(PCount) from tProduction where date_format(PDate, '%Y-%m') = '2020-02' group by INumber order by sum(PCount);
...
select ENumber, sum(PCount) from tProduction where PCount >= 500 group by ENumber order by sum(PCount) desc;
#7. 고객별 거래 빈도를 파악하고자 한다. 주문을 2번 이상 한 고객들의 고객번호와 주문 횟수를 구하시오. (고객번호 오름차순 정렬)
select CNumber, count(ONumber) from tOrder group by CNumber having count(ONumber)>=2 order by CNumber;
...
select RRNumber, count(ONumber), sum(RCount) from tReturn group by RRNumber;
...
select INumber, PCount from tProduction where PCount >= 500 and PDate >= 2020-05 order by PCount desc;
#13. 부서별 인사 현황을 파악하기 위해 각 부서의 인원수를 조사하려고 한다. 소속 부서가 있는 직원들을 대상으로 부서코드별 직원 수를 구하되, 직원 수가 5명 이상인 부서만 출력하시오.
select count(EName), DNumber from tEmployee where DNumber is not NULL group by DNumber having count(EName) >= 5;
#14. 2020년 3월 한 달 동안의 직원별 생산 활동을 분석하려고 한다. 직원 코드별 생산 건수와 평균 생산량을 출력하시오. 평균 생산량이 높은 순서대로 정렬하시오.
select ENumber, count(PNumber), avg(PCount) from tProduction group by ENumber order by avg(PCount) desc;
...
select INumber, count(INumber) from tProduction group by INumber having count(INumber) >= 3 order by INumber;