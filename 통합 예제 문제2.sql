...
select EName,date_format(StartDate,'%Y-%m-%d') from tEmployee where DNumber like 'D1001' or DNumber like 'D2001' order by StartDate;
#2. 제품 별로 생산량 최저, 최고, 총, 평균을 구하라. (min(), max(), sum(), avg())
    select INumber, min(PCount),max(PCount),count(PCount),avg(PCount) from tProduction group by INumber;
...
#9. 각 반품 사유별로 얼마나 많은 반품이 발생했는지 파악하고자 한다. 반품사유코드별 반품 건수와 총 반품 수량을 출력하시오.
    select count(*),sum(RCount) from tReturn group by RRNumber;
...
#12. 2020년 5월 이후 생산된 제품 중 총 생산량이 500 이상인 제품을 선별하려고 한다. 제품번호와 총 생산량을 출력하되, 총 생산량이 높은 순으로 정렬하시오.
    select INumber, sum(PCount) from tProduction where PDate >= '2020-06-01' group by INumber having sum(PCount) >= 500 order by sum(PCount) desc;
...
select INumber from tProduction group by INumber having count(*)>=3 order by INumber;