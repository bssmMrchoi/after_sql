...
select EName, StartDate from tEmployee where DNumber like 'D1001' or DNumber like 'D2001' order by StartDate;

--2. 제품 별로 생산량 최저, 최고, 총, 평균을 구하라. (min(), max(), sum(), avg())
select min(PCount), max(PCount), sum(PCount), avg(PCount) from tProduction group by PCount;

--3. 생산량 조정을 위해 2020년 2월의 총 생산량을 알려고 한다. 해당 월에 생산된 제품들의 코드와 해당 제품들의 총 생산량을 출력하시오. (총 생산량의 오름차순 출력) - sum()
select INumber, sum(PCount) from tProduction where PDate like '2020-01%' group by INumber, PCount order by sum(PCount);
...
select ENumber, sum(PCount) from tProduction group by ENumber having sum(PCount) >= 500 order by sum(PCount) desc
...
--10. 고가 제품 위주로 정리를 위해 단가가 1000원 이상인 제품들만 선별한 뒤, 제품군(제품번호 앞 2자리 기준)별 평균 단가를 출력하시오.substr
