...
select c.custid, count(t.targetid) from Target t join Order_Kill ok on t.targetid=ok.targetid join Customer c on ok.custid=c.custid
group by c.custid;

#2. 고객별 청부 금액이 50000원을 초과하는 청부를 몇번 넣었는지, 50000원 초과 청부의 가격 총 합을 구하시오
select c.custid, t.price, count(c.custid), sum(t.price)
from Customer c join Order_Kill ok on c.custid=ok.custid join Target t on t.targetid=ok.targetid
...
from Order_Kill
where targetid
in (select targetid from Order_Kill group by targetid having count(*)>=2);
